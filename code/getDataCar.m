function data = getDataCar(imname, imset, whatdata)

% example to run: data = getDataCar('000120', 'train', 'left');
% to load a detector, e.g.: data = getDataCar([],[],'detector-car');

if nargin < 2
    fprintf('run with: data = gt-left(imname, imset, whatdata);\n');
    fprintf('where:\n');
...
switch whatdata
    case {'list'}
        fid = fopen(fullfile(DATA_DIR_CAR, imset, [imset '.txt']), 'r+');
        ids = textscan(fid, '%s');
        ids = ids{1};
        fclose(fid);
        data.ids = ids;
    case {'left', 'left-plot', 'right', 'right-plot'}
        ...
        imfile = fullfile(DATA_DIR_CAR, imset, leftright, sprintf('%s.png', imname));
        im = imread(imfile);
        data.im = im;
    ...
    case 'calib'
        ...
        calib_dir = fullfile(DATA_DIR_CAR, imset, 'calib');
    ...
    case {'gt-left', 'gt-right', 'gt-left-plot', 'gt-right-plot', 'gt-left-plot-3d', 'gt-right-plot-3d'}
        ...
        label_dir = fullfile(DATA_DIR_CAR, imset, sprintf('gt_%s', leftright));
        img_idx = str2num(imname);
        objects = readLabels(label_dir,img_idx);
        data.objects = objects;
        calibdata = getDataCar(imname, imset, 'calib');
        ...
        if strcmp(whatdata, sprintf('gt-%s-plot%s', leftright, which))
           imgdata = gt-left(imname, imset, leftright);
           seg = [];
    ...
    case 'ds'
        ...
        class = {'car'};
        data.class = class;
        dsfile = fullfile(dispdir, sprintf('%s_car.mat', imname));
        data.dss{1} = load(dsfile);
    otherwise 
