...
function H=HOGdescriptor(img)

img = double(rgb2gray(img));
Im = [img zeros([size(img,1) 1])];
Im = [Im; zeros([1 size(Im,2)])];

nwin_x=size(Im,2)-1;%set here the number of HOG windows per bound box
nwin_y=size(Im,1)-1;
